...
zashhomo sub edit             Open config file in editor
zashhomo sub interval [dur]   View/set global refresh interval (e.g. 6h, 30m)
zashhomo sub update           Regenerate config and hot-reload kernel
...
Subscriptions are written to mihomo config as `proxy-providers`, generating `PROXY` (select) and `AUTO` (url-test) proxy groups; then hot-reloaded, nodes visible in panel.
...
Data directory contains: `bin/` (mihomo binary), `ui/` (zashboard static), `providers/` (subscription cache), `config.yaml` (generated mihomo config), `zashhomo.log`.
...
sub_interval: 12h                 # Subscription refresh interval
