...
- **Subscription/Config Management**: Profile model — each subscription cached separately, only one active at a time; supports offline switching, per-subscription refresh intervals, and scheduled update toggles.
- **System Proxy Toggle**: `zashhomo system-proxy enable|disable` sets/clears OS system proxy cross-platform (Windows registry, macOS networksetup, Linux GNOME gsettings); state persisted and synced on service start/stop.
- **TUN Mode Persistence**: TUN toggles in the panel are synced to `zashhomo.yaml` and survive restarts.
- **Interactive Menu**: `zashhomo -i` launches a TUI management interface (arrow-key selection) with status overview, subscription management, and guided setup.
...
> **Note**: Current releases only ship Windows prebuilt binaries (amd64/arm64). Linux/macOS users please build from source (see below), or wait for future releases to enable those platforms.
...
| --- | --- | --- |
| Linux/macOS (user) | `~/.local/share/zashhomo` (or `$XDG_DATA_HOME/zashhomo`) | `~/.config/zashhomo/zashhomo.yaml` (or `$XDG_CONFIG_HOME/zashhomo/zashhomo.yaml`) |
| Linux/macOS (root) | `/var/lib/zashhomo` | `/etc/zashhomo/zashhomo.yaml` |
| Windows | `%ProgramData%\zashhomo` | Same as data directory |

Data directory contains: `bin/` (mihomo binary), `ui/` (zashboard static), `subs/` (raw subscription files, named by id), `config.yaml` (mihomo config generated from the active subscription), `zashhomo.log`.

Environment variables:
- `ZASHHOMO_DATA` — data directory
- `ZASHHOMO_CONFIG_DIR` — config directory
- `ZASHHOMO_BIN` — install directory (where the zashhomo executable lives)
- `ZASHHOMO_STATE_DIR` — state directory (stores UI state like onboarding marker)
...
system_proxy: false               # Whether to auto-manage system proxy (set by system-proxy command)
...
tun:                              # TUN mode config (synced from panel toggle, auto-persisted)
  enable: false
  stack: system
...
- Depends on stdlib + `gopkg.in/yaml.v3` + `github.com/kardianos/service` + `github.com/charmbracelet/bubbletea` (interactive menu), no heavy CLI frameworks like cobra; CLI subcommands are hand-written dispatch, `CGO_ENABLED=0` for static binary + `-ldflags "-s -w"` for size reduction.
...
Pushing `v*` tag triggers `.github/workflows/release.yml`: currently builds windows(amd64/arm64) only, uploads `zashhomo-<version>-windows-<arch>.exe` + `SHA256SUMS.txt` to Releases.
Linux/macOS builds are temporarily disabled; will be enabled in future releases.
